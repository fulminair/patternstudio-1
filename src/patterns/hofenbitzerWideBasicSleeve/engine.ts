...
import type {
  ...
  HofenbitzerWideBasicSleeveDerivedValues,
  HofenbitzerWideBasicSleeveDraftMetrics,
...
const DEFAULT_BASE_MEASUREMENTS: HofenbitzerWideBasicSleeveBaseMeasurements = {
  AhH: 20,
  AhHEase: 0,
  fAh: 19.6,
  bAh: 23.9,
...
const calculateMark3RadiusCm = (derived: HofenbitzerWideBasicSleeveDerivedValues): number => {
  const radius = 0.97 * derived.fAhConstruction + 0.25 * derived.CapEaseCm;
  return isFinite(radius) && radius > 0 ? radius : 0;
};

const calculateMark4RadiusCm = (derived: HofenbitzerWideBasicSleeveDerivedValues): number => {
  const radius = 0.97 * derived.bAhConstruction + 0.75 * derived.CapEaseCm;
  return isFinite(radius) && radius > 0 ? radius : 0;
};

const buildMark3Arc = (
  center: DraftPoint,
  derived: HofenbitzerWideBasicSleeveDerivedValues,
  targetLengthCm: number,
): ArcInfo | null => {
...
const computeCapMarkerPoints = (
  ...
  return {
    ...
    mark13: p(mark2.x - capLineCm / 14, mark2.y),
...
type Geometry = {
  ...
  metrics: HofenbitzerWideBasicSleeveDraftMetrics;
};

const computeGeometry = (
  measurements: HofenbitzerWideBasicSleeveEffectiveMeasurements,
): Geometry => {
  const points: Record<string, DraftPoint> = {};

  const baselineLength = measurements.SlW + measurements.CapLineEase;
  const mark1 = p(0, 0);
...
export const getDefaultBaseMeasurements = (): HofenbitzerWideBasicSleeveBaseMeasurements => ({
  ...DEFAULT_BASE_MEASUREMENTS,
});

export const computeDerived = (
  data: HofenbitzerWideBasicSleeveBaseMeasurements,
): HofenbitzerWideBasicSleeveDerivedValues => {
  ...
  const capLineCm = SlW + asNumber(data.CapLineEase);
...
export const computeDraftMetrics = (
  measurements: HofenbitzerWideBasicSleeveEffectiveMeasurements,
): HofenbitzerWideBasicSleeveDraftMetrics => computeGeometry(measurements).metrics;
