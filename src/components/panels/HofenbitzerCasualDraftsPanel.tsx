...
import type { HofenbitzerCasualPatternInstance } from "@/patterns/hofenbitzerCasualBodice/types";
...
function DraftListItem({
  ...
  draft: HofenbitzerCasualPatternInstance;
  selected: boolean;
}) {
  const setSelectedInstance = useHofenbitzerCasualStore((state) => state.setSelectedInstance);
  const toggleVisible = useHofenbitzerCasualStore((state) => state.toggleVisible);
  const removeInstance = useHofenbitzerCasualStore((state) => state.removeInstance);
  const setInstanceColor = useHofenbitzerCasualStore((state) => state.setInstanceColor);
...
export function HofenbitzerCasualDraftsPanel() {
  const drafts = useHofenbitzerCasualStore((state) => state.instances);
  const selectedDraftId = useHofenbitzerCasualStore((state) => state.ui.selectedInstanceId);
  const addDuplicate = useHofenbitzerCasualStore((state) => state.addDuplicate);
