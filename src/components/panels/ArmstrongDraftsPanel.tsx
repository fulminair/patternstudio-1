...
import type { ArmstrongPatternInstance } from "@/patterns/armstrongBodice/types";
...
function DraftListItem({
  ...
  draft: ArmstrongPatternInstance;
  selected: boolean;
}) {
  const setSelectedInstance = useArmstrongStore((state) => state.setSelectedInstance);
  const toggleVisible = useArmstrongStore((state) => state.toggleVisible);
  const removeInstance = useArmstrongStore((state) => state.removeInstance);
  const setInstanceColor = useArmstrongStore((state) => state.setInstanceColor);
...
export function ArmstrongDraftsPanel() {
  const drafts = useArmstrongStore((state) => state.instances);
  const selectedDraftId = useArmstrongStore((state) => state.ui.selectedInstanceId);
  const addDuplicate = useArmstrongStore((state) => state.addDuplicate);
