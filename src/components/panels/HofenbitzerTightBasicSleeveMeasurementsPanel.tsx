...
import { useHofenbitzerTightBasicSleeveStore } from "@/lib/hofenbitzerTightBasicSleeveStore";
...
export function HofenbitzerTightBasicSleeveMeasurementsPanel() {
  ...
  const selectedInstanceId = useHofenbitzerTightBasicSleeveStore((state) => state.ui.selectedInstanceId);
  const setOverride = useHofenbitzerTightBasicSleeveStore((state) => state.setOverride);
  const resetOverrides = useHofenbitzerTightBasicSleeveStore((state) => state.resetOverrides);

  const selectedDraft =
    instances.find((instance) => instance.id === selectedInstanceId) ?? instances[0] ?? null;

  const selectedMeasurements = useMemo<HofenbitzerTightBasicSleeveBaseMeasurements>(() => {
    if (!selectedDraft) {
      return base;
    }
  ...
  const metrics = useMemo(() => {
    const effective: HofenbitzerTightBasicSleeveEffectiveMeasurements = {
  ...
  return (
    <section className="space-y-3">
      <div className="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs leading-relaxed text-slate-600">
        <p className="font-semibold text-slate-700">Hofenbitzer&apos;s Tight Basic Sleeve</p>
        <p>
          All measurements are in centimetres. Defaults reference the size 38 tight sleeve block from
