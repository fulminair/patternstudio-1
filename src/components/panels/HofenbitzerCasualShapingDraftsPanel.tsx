...
import type { HofenbitzerCasualShapingPatternInstance } from "@/patterns/hofenbitzerCasualShapingBodice/types";
...
function DraftListItem({
  ...
  draft: HofenbitzerCasualShapingPatternInstance;
  selected: boolean;
}) {
  const setSelectedInstance = useHofenbitzerCasualShapingStore((state) => state.setSelectedInstance);
  const toggleVisible = useHofenbitzerCasualShapingStore((state) => state.toggleVisible);
  const removeInstance = useHofenbitzerCasualShapingStore((state) => state.removeInstance);
  const setInstanceColor = useHofenbitzerCasualShapingStore((state) => state.setInstanceColor);
...
export function HofenbitzerCasualShapingDraftsPanel() {
  const drafts = useHofenbitzerCasualShapingStore((state) => state.instances);
  const selectedDraftId = useHofenbitzerCasualShapingStore((state) => state.ui.selectedInstanceId);
  const addDuplicate = useHofenbitzerCasualShapingStore((state) => state.addDuplicate);
