...
import { useHofenbitzerCasualStore } from "@/lib/hofenbitzerCasualStore";

type EditableKey = Exclude<keyof HofenbitzerCasualBaseMeasurements, "FitIndex">;
...
type TripletRow = {
  ...
  ease: EditableKey;
  derivedLabel: string;
  derivedValue: (derived: HofenbitzerCasualDerivedValues) => number;
};
...
export function HofenbitzerCasualMeasurementsPanel() {
  const base = useHofenbitzerCasualStore((state) => state.base);
  const instances = useHofenbitzerCasualStore((state) => state.instances);
  const selectedInstanceId = useHofenbitzerCasualStore((state) => state.ui.selectedInstanceId);
  const setOverride = useHofenbitzerCasualStore((state) => state.setOverride);
  const applyFitProfileToDraft = useHofenbitzerCasualStore((state) => state.applyFitProfileToDraft);
  const resetOverrides = useHofenbitzerCasualStore((state) => state.resetOverrides);

  const selectedDraft =
    instances.find((instance) => instance.id === selectedInstanceId) ?? instances[0] ?? null;

  const selectedMeasurements = useMemo<HofenbitzerCasualBaseMeasurements>(() => {
    if (!selectedDraft) {
      return base;
    }
  ...
  const commitMeasurement = (key: keyof HofenbitzerCasualBaseMeasurements, value: number) => {
    if (!selectedDraft) {
      return;
    }
  ...
  return (
    <section className="space-y-3">
      <div className="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs leading-relaxed text-slate-600">
        <p className="font-semibold text-slate-700">Hofenbitzer&apos;s Casual Bodice</p>
        <p>
