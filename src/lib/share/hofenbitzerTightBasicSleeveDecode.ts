import { getDefaultBaseMeasurements } from "@/patterns/hofenbitzerTightBasicSleeve/engine";
import type {
...
const MEASUREMENT_KEYS: Array<keyof HofenbitzerTightBasicSleeveBaseMeasurements> = [
  "AhH",
  "AhHEase",
  "fAh",
  "bAh",
...
const parseBase = (
  input: unknown,
  defaults: HofenbitzerTightBasicSleeveBaseMeasurements,
): HofenbitzerTightBasicSleeveBaseMeasurements => {
  if (!isObject(input)) {
    return defaults;
  }
...
const parseInstances = (input: unknown): HofenbitzerTightBasicSleevePatternInstance[] => {
  ...
  return input
    ...
    .map((value, index) => {
      ...
      const parsedOverrides: HofenbitzerTightBasicSleevePatternInstance["overrides"] = {};
...
const parseUi = (
  ...
  const fallback: HofenbitzerTightBasicSleeveProjectUiState = {
...
export const decodeHofenbitzerTightBasicSleeveProjectState = (
  encoded: string,
): HofenbitzerTightBasicSleeveProjectState | null => {
  try {
