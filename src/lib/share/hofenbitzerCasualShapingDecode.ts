import { getDefaultBaseMeasurements } from "@/patterns/hofenbitzerCasualShapingBodice/engine";
import type {
...
const MEASUREMENT_KEYS: Array<keyof HofenbitzerCasualShapingBaseMeasurements> = [
  "FitIndex",
  "AhD",
  "AhDEase",
  "BrC",
...
const parseBase = (
  input: unknown,
  defaults: HofenbitzerCasualShapingBaseMeasurements,
): HofenbitzerCasualShapingBaseMeasurements => {
  if (!isObject(input)) {
    return defaults;
  }
...
const parseInstances = (input: unknown): HofenbitzerCasualShapingPatternInstance[] => {
  ...
  return input
    ...
    .map((value, index) => {
      ...
      const parsedOverrides: HofenbitzerCasualShapingPatternInstance["overrides"] = {};
...
const parseUi = (
  ...
  const fallback: HofenbitzerCasualShapingProjectUiState = {
...
export const decodeHofenbitzerCasualShapingProjectState = (
  encoded: string,
): HofenbitzerCasualShapingProjectState | null => {
  try {
