import { getDefaultBaseMeasurements } from "@/patterns/hofenbitzerContouredBodiceHipGap/engine";
import type {
...
const MEASUREMENT_KEYS: Array<keyof HofenbitzerContouredHipGapBaseMeasurements> = [
  "FitIndex",
  "AhD",
  "AhDEase",
  "BrC",
...
const parseBase = (
  input: unknown,
  defaults: HofenbitzerContouredHipGapBaseMeasurements,
): HofenbitzerContouredHipGapBaseMeasurements => {
  if (!isObject(input)) {
    return defaults;
  }
...
const parseInstances = (input: unknown): HofenbitzerContouredHipGapPatternInstance[] => {
  ...
  return input
    ...
    .map((value, index) => {
      ...
      const parsedOverrides: HofenbitzerContouredHipGapPatternInstance["overrides"] = {};
...
const parseUi = (
  ...
  const fallback: HofenbitzerContouredHipGapProjectUiState = {
...
export const decodeHofenbitzerContouredHipGapProjectState = (
  encoded: string,
): HofenbitzerContouredHipGapProjectState | null => {
  try {
